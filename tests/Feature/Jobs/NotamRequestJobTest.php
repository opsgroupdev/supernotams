...
it('stores received notams to the database and triggers tagging jobs', function () {
    ...
    Http::fakeSequence()->push(File::get(base_path('tests/source/Trial_Run.json')));
    ...
    $allNotams = Notam::all();
    expect($allNotams)->toHaveCount(56);
    expect($allNotams->pluck('status'))->each()->toBe(NotamStatus::PROCESSING);
    Bus::assertDispatchedTimes(NotamTagJob::class, 56);
});
...
it('does not DUPLICATE notams already in the database', function () {
    ...
    Http::preventStrayRequests();
    Http::fakeSequence()
        ->pushFile(base_path('tests/source/Trial_Run.json'))
        ->pushFile(base_path('tests/source/Trial_Run.json'));

    expect(Notam::count())->toBe(0);
    $job = new NotamRequestJob('eidw,einn');
    $job->handle();
    expect(Notam::count())->toBe(56);
...
it('does not UPDATE the structure of a notam if it already exists', function () {
    ...
    Storage::fake();
    Http::preventStrayRequests();
    Http::fakeSequence()->pushFile(base_path('tests/source/Trial_Run.json'));
    ...
    $updatedNotam = Notam::find('A0414/24-EIDW');
